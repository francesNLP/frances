...
with open (os.path.join(input_path, 'topics_1ed.txt'), 'rb') as fp5:
    topics = pickle.load(fp5)

with open (os.path.join(input_path, 't_names_1ed.txt'), 'rb') as fp6:
    t_names = pickle.load(fp6)

with open (os.path.join(input_path, 'topics_names_1ed.txt'), 'rb') as fp7:
    topics_names = pickle.load(fp7)

model = SentenceTransformer('bert-base-nli-mean-tokens')

topic_model = BERTopic.load("/Users/rosafilgueira/HW-Work/NLS-Fellowship/work/frances/web-app/models/BerTopic_Model_1ed") 
...
def term_search(termlink=None):
    ...
    headers=["Year", "Edition", "Volume", "Start Page", "End Page", "Term Type", "Definition", "Related Terms", "Topic Modelling"]
...
def similar_terms(termlink=None):
    if request.method == "POST":
        if 'resource_uri' in request.form:
            ...
            if "https://" in data_similar or "w3id" in data_similar:
                ...
                uri_raw=""
                results, topics_vis=calculating_similarity_text(data_similar,text_embeddings, model, terms_info, documents,uris, topics_names,topics)              
                if len(topics_vis) >= 1:
                    fig1=topic_model.visualize_barchart(topics_vis, n_words=10)
                    bar_plot = fig1.to_json() 
                else:
            ...
            if uri_raw!="":
                ...
                for i in range(-2, -12, -1):
                    ...
                    results[uris[similar_index]]=[terms_info[similar_index][1],terms_info[similar_index][2], terms_info[similar_index][4], terms_info[similar_index][0], documents[similar_index], topic_name, rank]
                if len(topics_vis) >= 1:
                    fig1=topic_model.visualize_barchart(topics_vis, n_words=10)
                    bar_plot = fig1.to_json()
                else:
            ...
            if uri_raw == "":
                ...
                return render_template('similar.html', results=results, term=term, definition=definition, uri=uri_raw, enum=enum, year=year, vnum=vnum, t_name=t_name, bar_plot=bar_plot, heatmap_plot=heatmap_plot)
    ...
    if termlink!=None:
        ...
        if len(topics_vis) >= 2:
            ...
            heatmap_plot = None
        return render_template('similar.html', results=results, term=term, definition=definition, uri=uri, enum=enum, year=year, vnum=vnum, t_name=t_name, bar_plot=bar_plot, heatmap_plot=heatmap_plot)
...
def topic_modelling(topic_name=None):
    if request.method == "POST":
        if 'topic_name' in request.form:
            ...
            if topic_name=="":
                topic_name="6_see_dialling_mahometans_bildge"
            indices = [i for i, x in enumerate(topics_names) if x == topic_name]
            results={}
            for t_i in indices:
               results[uris[t_i]]=[terms_info[t_i][1],terms_info[t_i][2], terms_info[t_i][4], terms_info[t_i][0], documents[t_i]]
